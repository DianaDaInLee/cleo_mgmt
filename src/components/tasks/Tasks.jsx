...
export default function Tasks() {
  ...
  const startEdit = (t) => {
    setEditingId(t.id)
    setEditValues({ name: t.name, assignee: t.assignee || '', dueDate: t.dueDate || '', priority: t.priority || 'Medium', status: t.status || 'Not Started', notes: t.notes || '' })
  }
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-800/60 border-b border-gray-700">
                  ...
                  <th className="text-left px-4 py-2.5 text-xs font-semibold text-gray-400 uppercase tracking-wider w-36">Assignee</th>
                  <th className="text-left px-4 py-2.5 text-xs font-semibold text-gray-400 uppercase tracking-wider w-32">Due Date</th>
                  <th className="text-left px-4 py-2.5 text-xs font-semibold text-gray-400 uppercase tracking-wider w-28">Priority</th>
          ...
          <div className="card w-full max-w-lg">
            ...
            <form onSubmit={handleAdd} className="space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-1">
                <button
                  type="button"
                  onClick={() => { setShowModal(false); setNewTask(EMPTY_TASK) }}
                  className="btn-ghost"
                >
                  Cancel
                </button>
